...
export function buildCategoryMonthlyTrend(
  transactions: DashboardTransaction[],
  categoryBudgets: Record<string, number>
): Record<string, MonthlyBudgetPoint[]> {
  const result: Record<string, MonthlyBudgetPoint[]> = {};
  ...
  for (const category of categories) {
    ...
    for (const tx of sorted) {
      const label = `${Number(tx.date.slice(5, 7))}월`;
      byMonth.set(label, (byMonth.get(label) ?? 0) + tx.amount);
    }

    const points: MonthlyBudgetPoint[] = [...byMonth.entries()].map(([month, used]) => ({
      month,
      used,
    ...
    result[category] = enrichTrendWithMoM(points);
  }
