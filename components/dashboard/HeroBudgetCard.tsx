...
export default function HeroBudgetCard({
  ...
  const total = emptyData ? 0 : BUDGET_TOTAL;
  const used = emptyData ? 0 : BUDGET_USED;
  const remaining = emptyData ? 0 : BUDGET_REMAINING;
  const usagePercent = emptyData ? 0 : BUDGET_USAGE_PERCENT;
  const usageSpeedPercent = emptyData ? 0 : BUDGET_USAGE_SPEED_PERCENT;
  const slices = emptyData ? [] : BUDGET_SLICES;
  const centerPercent = emptyData ? 0 : DOUGHNUT_CENTER_PERCENT;
