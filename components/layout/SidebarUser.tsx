...
import AccountantProfileModal from "@/components/layout/AccountantProfileModal";
...
export default function SidebarUser({ onAddGroup }: SidebarUserProps) {
  ...
  return (
    <>
        ...
        <div className="flex justify-center px-1.5 py-1">
          {isAccountant ? (
            <button
              ...
              className="rounded-full transition-transform duration-200 ease-premium hover:scale-[1.04]"
              aria-label="프로필 열기"
            >
      ...
      {isAccountant && (
        <AccountantProfileModal open={profileOpen} onClose={() => setProfileOpen(false)} />
      )}
    </>
  );
}
